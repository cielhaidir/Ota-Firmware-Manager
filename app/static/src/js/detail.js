function openNodeConfig(nodeName, mac, user_id) {
  ...
  fetch(`../source/${user_id}_${nodeName}.json`)
...
DeleteButtons.forEach(function (button) {
  ...
  function createCheckbox(labelText, id, isChecked, nodeName) {
    ...
    checkbox.id = id;
    checkbox.className = 'toggle mt-5 ms-5';
...
detailButtons.forEach(function (button) {
  button.addEventListener('click', function () {
    ...
    openNodeConfig(nodeName, macAddr, user_id);

    const switchDiv = document.querySelector(`.switch-container[data-node-name="${nodeName}"]`);
    switchDiv.innerHTML = '';

    fetch(`/get_config/${user_id}_${nodeName}`)
      .then(response => response.json())
      .then(data => {
        console.log(data);
        if (data.status === 'success' && data.config && data.config.main) {
          for (const property in data.config.main) {
            if (data.config.main.hasOwnProperty(property)) {
              console.log('Creating Checkbox for Property:', property);
              const checkbox = createCheckbox(property, property.toLowerCase(), data.config.main[property], nodeName);
              switchDiv.appendChild(checkbox);

              // Add event listener for checkbox change
              checkbox.querySelector('.toggle').addEventListener('change', function () {
                const propertyStatus = this.checked ? 'ON' : 'OFF';
                const configName = `${user_id}_${nodeName}`;
